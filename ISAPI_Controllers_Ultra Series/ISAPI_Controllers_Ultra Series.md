...
Ultra Series
```
DS-K2700, DS-K2701X, DS-K2701X(P), DS-K2702WX-E1, DS-K2702WX-E1(P), DS-K2702X, DS-K2702X(P), DS-
```
```
K2704X, DS-K2704X(P), DS-K2708X, DS-K27M
```
...
10 API Reference
```
10.1 Device Management (General)
```
10.1.1 Event Subscription Management
10.1.1.1 Set alarm/event subscription parameters
Hikvision co MMC
adil@hikvision.co.az
...
None
Response Message
10.1.1.2 Get alarm/event subscription parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<SubscribeEvent xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
```
10.1.1.3 Subscribe to events/alarms in arming mode
Hikvision co MMC
adil@hikvision.co.az
```
"alarmResult", "attendance", "attendedBaggage", "audioAbnormal", "audioexception", "behaviorResult"(abnormal event detection), "blindSpotDetection"(blind
...
None
Response Message
10.1.1.4 Get the alarm/event subscription capability
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
Query Parameter
10.1.2 Online Upgrading Management
10.1.2.1 Get the device online upgrade capability
Hikvision co MMC
adil@hikvision.co.az
None
Request Message
...
10.1.2.2 Get the online upgrade package downloading progress
10.1.2.3 Start to download upgrade package to device
10.1.2.4 Get the online upgrade progress of device
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
None
...
Request URL
GET /ISAPI/System/onlineUpgrade/status/task/<taskID>
Query Parameter
Parameter
Name
Parameter
...
taskID string
```
in the URL refers to the task ID, which is returned by the URL (POST
```
```
/ISAPI/System/onlineUpgrade/task?format=json). It is to get the upgrade status of internal
```
sub modules or sub channels of devices at the same time.
Request Message
None
Response Message
10.1.2.5 Get the progress of a specified online upgrade task
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<OnlineUpgradeStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
```
<!--ro, req, object, online upgrade status, attr:version{req, string, protocolVersion}-->
```
<status>
```
<!--ro, req, enum, online upgrade status, subType:string, desc:"notUpgrade" (not upgraded), "upgrading", "successful" (upgraded), "languageMismatch"
...
```
```
version mismatch), "netUnreachable" (network disconnected), "unknownError" (unknown error), "waitManualTrigger" (waiting for manual trigger; this status
```
```
will be returned if delayed upgrade is set when the task is applied and not triggered manually)-->notUpgrade
```
</status>
<percent>
<!--ro, req, int, current upgrade progress in percentage, range:[0,100]-->1
...
<taskID>
<!--ro, opt, string, task ID, range:[1,64], desc:generated when creating upgrade tasks, related URI: /ISAPI/System/onlineUpgrade/task?format=json-->test
</taskID>
<moudleType>
```
<!--ro, opt, enum, component type, subType:string, desc:"stationHostAndScreen" (dock station upgrade package, including host and screen), "stationHost"
```
```
(dock station host), "bodyCamera" (body camera)-->stationHostAndScreen
```
</moudleType>
<packageName>
<!--ro, opt, string, upgrade package name, range:[1,128]-->ZCS-1_target_V1.0.1_build201123.zip
</packageName>
<taskTime>
<!--ro, opt, datetime, time of applying the upgrade task-->1970-01-01T00:00:00+08:00
</taskTime>
</OnlineUpgradeStatus>
Request URL
...
```
Request URL
POST /ISAPI/System/onlineUpgrade/task?format=json
10.1.2.6 Get all of the online upgrade task progress
10.1.2.7 Apply the task of online upgrade
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
None
...
```
/*opt, enum, module type, subType:string, desc:“cardReader” (card reader), "FPModule" (fingerprint module), "securityModule" (security module),
```
```
...
None
Response Message
10.1.3 Serial Port Management
10.1.3.1 Get the serial port capability of the device
10.1.3.2 Get the capability of a specific serial portHikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
10.1.3.3 Set the parameters of a specific serial port supported by the device
Hikvision co MMC
adil@hikvision.co.az
```
...
Query Parameter
10.1.3.4 Get the parameters of a specific port supported by the device
10.1.3.5 Get the list of serial ports supported by the device
Hikvision co MMC
adil@hikvision.co.az
Parameter
Name
...
10.1.4.1 Get the storage capability of the device
10.1.4.2 Get the system security capability
Hikvision co MMC
adil@hikvision.co.az
...
10.1.4.3 Get device system capability
Hikvision co MMC
adil@hikvision.co.az
</isSupportWired>
<isSupportPPPoE>
...
<isSupportResetBatchChild>
<!--ro, opt, bool, whether it supports batch restoring sub-devices, desc:PUT /ISAPI/System/ResetBatchChild?format=json-->true
</isSupportResetBatchChild>
<isSupportSearchResetBatchChildStatus>
<!--ro, opt, bool, whether it supports searching for the status of batch restoring sub-devices, desc:POST /ISAPI/System/SearchResetBatchChildStatus?
```
format=json-->true
```
</isSupportSearchResetBatchChildStatus>
...
<isSupportGetUserManualLink>
<!--ro, opt, bool, whether it supports getting the device user manual link, desc:GET /ISAPI/System/GetUserManualLink?format=json-->true
</isSupportGetUserManualLink>
</DeviceCap>
Request URL
...
Response Message
10.1.4.4 Get the languages supported by the device
10.1.4.5 Set device language parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<ResponseStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
None
Response Message
10.1.4.6 Get the capability of configuring the device language
10.1.4.7 Set restoring devices' default parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<ResponseStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
None
Response Message
10.1.4.8 Get the network service capability
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
10.1.5 Upgrade Management
10.1.5.1 Get the capabilities of peripheral module upgrade
Hikvision co MMC
adil@hikvision.co.az
</securityModuleNo>
...
None
Response Message
10.1.5.2 Get the device upgrading status and progress
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
10.1.6 Time Management
10.1.6.1 Get device time synchronization management parameters
10.1.6.2 Set device time synchronization management parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Response MessageHikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<ResponseStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
```
...
<isSupportTimeFilter>
10.1.6.3 Get the capability of device time synchronization management
Hikvision co MMC
adil@hikvision.co.az
<isSupportTimeFilter>
...
10.1.6.4 Set parameters of all NTP servers
```
10.1.6.5 Get the parameters of a specific NTP (Network Time Protocol) server
```
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
...
NTPServerID string --
Request Message
10.1.6.6 Set the parameters of a NTP server
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
10.1.6.7 Get the parameters of a NTP server
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<NTPServer xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
Response Message
```
10.1.6.8 Get the configuration capability of parameters of a specific NTP (Network Time Protocol) server
```
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
10.2.1 HTTP Listening Management
```
...
10.3 Video (General)
```
10.3.1 Motion Detection
10.3.1.1 Get arming schedule of motion detection of all channels
10.3.1.2 Set arming schedule of motion detection of all channels
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
"@opt": "80,90,112,113…"
```
10.4 Access Control (General)
```
10.4.1 Access Control Event Management
10.4.1.1 Get the capability of searching for access control events
Hikvision co MMC
adil@hikvision.co.az
"@opt": "80,90,112,113…"
...
None
Response Message
10.4.1.2 Get the configuration capability of storing access control events
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
10.4.1.3 Get the storage parameters of access control events
10.4.1.4 Set storage parameters of access control events
Hikvision co MMC
adil@hikvision.co.az
Response Message
```
...
"AcsEvent": {
```
10.4.1.5 Search for access control events
Hikvision co MMC
adil@hikvision.co.az
```
"AcsEvent": {
...
"endSerialNo": {
```
10.4.1.6 Get the capability of getting total number of access control events by specific conditions
Hikvision co MMC
adil@hikvision.co.az
/*ro, opt, object, end serial No.*/
"@min": 1,
...
```
Response Message
10.4.1.7 Get the capability of clearing event and card linkage parameters
10.4.1.8 Clear event card linkage configurations
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.1.9 Getting arming information
10.4.1.10 Getting arming information capability
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<DeployInfo xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
```
Response Message
10.4.1.11 Set the event card linkage parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.1.12 Get the event and card linkage configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
10.4.1.13 Get the configuration capability of the event and card linkage
Hikvision co MMC
adil@hikvision.co.az
```
},
...
```
10.4.1.14 Get the capability of the list of event and card linkage ID
10.4.1.15 Get the list of event and card linkage ID
10.4.1.16 Get the configuration capability of event optimization
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
None
Response Message
10.4.1.17 Set the event optimization parameters
10.4.1.18 Get the event optimization configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
"caseSensorNo": 1,
/*ro, opt, int, event trigger No.*/
10.4.1.19 Access control event
Hikvision co MMC
adil@hikvision.co.az
"RS485No": 1,
/*ro, opt, int, RS-485 channel No.*/
...
10.4.2 Access Point Status Schedule
10.4.2.1 Get the capability of clearing access control schedule configuration.
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
None
...
None
Request Message
10.4.2.2 Clear access control schedule configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
Response Message
10.4.2.3 Set holiday group parameters of door control schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.2.4 Get the holiday group configuration parameters of the door control schedule
10.4.2.5 Get the configuration capability of door status parameters of holiday group
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
10.4.2.6 Get the configuration parameters of the door control holiday schedule
10.4.2.7 Set parameters of door control holiday schedule
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
None
Response Message
10.4.2.8 Get the configuration capability of the door control holiday schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
Request URL
10.4.2.9 Get the configuration parameters of the door control schedule
10.4.2.10 Set parameters of door control schedule
Hikvision co MMC
adil@hikvision.co.az
...
Query Parameter
10.4.2.11 Get the configuration capability of the door control schedule
10.4.2.12 Set parameters of door control schedule template
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
...
```
10.4.2.13 Get parameters of door control schedule template
10.4.2.14 Get the configuration capability of the door control schedule template
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
weekPlanID string --
Request Message
10.4.2.15 Set parameters of door control weekly schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.2.16 Get the configuration parameters of the door control week schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.2.17 Get the configuration capability of the door control week schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.3 Access Control Device Management
10.4.3.1 Get the configuration capability of the access controller
Hikvision co MMC
adil@hikvision.co.az
```
{
...
"enable3G4G": true,
/*opt, bool, whether to enable 3G/4G, desc:whether to enable 3G/4G*/
"protocol": "Private",
```
...
```
/*opt, bool, dep:or,{$.AcsCfg.showPhoneNo,eq,true}*/
```
"thermalEnabled": true,
...
10.4.3.2 Set the parameters of the access controller
Hikvision co MMC
adil@hikvision.co.az
doors, this field does not need to be configured*/
...
```
/*opt, int, range:[1,10], unit:s, dep:or,{$.AcsCfg.remoteCheckDoorEnabled,eq,true}*/
```
"remoteCheckVerifyMode": 1,
```
/*opt, enum, subType:int, dep:or,{$.AcsCfg.remoteCheckDoorEnabled,eq,true}*/
```
...
```
/*opt, enum, subType:string, dep:or,{$.AcsCfg.checkChannelType,eq,ISAPIListen}*/
```
...
"thermalDoorEnabled": false,
10.4.3.3 Get the configuration parameters of the access controller
Hikvision co MMC
adil@hikvision.co.az
"thermalDoorEnabled": false,
...
Query Parameter
10.4.3.4 Get the capability of getting the working status of
Hikvision co MMC
adil@hikvision.co.az
None
Request Message
...
None
Response Message
10.4.3.5 Get the working status of the access controller
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Query Parameter
10.4.4 Access Control Module Management
10.4.4.1 Get the door configuration capability
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
...
```
10.4.4.2 Set the door (floor) parameters
```
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
...
```
10.4.4.3 Get the door (floor) configuration parameters
```
Hikvision co MMC
adil@hikvision.co.az
<!--ro, opt, int, alarm time of magnetic contact detection timeout, range:[0,255], unit:s, desc:0 refers to not triggering alarm-->1
...
```
10.4.5 Anti-Passback
10.4.5.1 Get the anti-passing back configuration capability
10.4.5.2 Get the parameters of anti-passback configuration
10.4.5.3 Set the anti-passing back parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Query Parameter
10.4.5.4 Get the anti-passing back configuration parameters of a specified card reader
10.4.5.5 Set anti-passing back parameters of a card reader
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
cardReaderID string --
...
```
Request URL
10.4.5.6 Get the configuration capability of anti-passing back parameters of card readers
10.4.5.7 Get the capability of clearing anti-passback records
Hikvision co MMC
adil@hikvision.co.az
...
```
10.4.5.8 Clear anti-passback records in the device
10.4.5.9 Get the capability of clearing anti-passback parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Query Parameter
10.4.5.10 Clear anti-passing back parameters
10.4.6 Authentication Schedule Management
10.4.6.1 Set control schedule parameters of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
cardReaderID string --
...
None
Response Message
10.4.6.2 Get the control schedule configuration parameters of the card reader authentication mode
10.4.6.3 Get the control schedule configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Query Parameter
10.4.6.4 Get the holiday group configuration parameters of the control schedule of the card reader
authentication mode
10.4.6.5 Set holiday group parameters of control schedule of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
holidayGroupID string --
...
```
10.4.6.6 Get the holiday group configuration capability of the control schedule of the card reader
authentication mode
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
holidayPlanID string --
Request Message
10.4.6.7 Get holiday schedule parameters of the card reader authentication mode
10.4.6.8 Set holiday schedule parameters of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.6.9 Get the holiday schedule configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Request Message
None
10.4.6.10 Get the schedule template parameters of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
Response Message
```
...
None
Request Message
10.4.6.11 Set the schedule template parameters of the card reader authentication mode
10.4.6.12 Get the schedule template configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
None
Response Message
10.4.6.13 Get the week schedule configuration parameters of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
weekPlanID string Weekly schedule No., which starts from 1, and the maximum value supported by thedevice is obtained from the capability set.
Request Message
10.4.6.14 Set the week schedule parameters of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.6.15 Get the weekly schedule configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
cardReaderID string --
Request Message
10.4.7 Credential Recognition Module Management
10.4.7.1 Set the card reader parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
cardReaderID string --
Request Message
10.4.7.2 Get the card reader configuration parameters
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
None
Response Message
10.4.7.3 Get the configuration capability of the card reader
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
"localExtensionCardReaderInfoList": [
/*ro, opt, array, subType:object*/
```
{
```
...
10.4.7.4 Get the capability of configuring card No. authentication rule
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
None
...
None
Request Message
10.4.7.5 Get the switching progress and configuration result of card No. authentication mode
10.4.7.6 Get the parameters of card No. authentication mode
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
```
10.4.7.7 Set the parameters of card No. authentication mode
```
10.4.7.8 Get the configuration capability of enabling NFC (Near-Field Communication) function
```
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Response Message
```
10.4.7.9 Get the parameters of enabling NFC (Near-Field Communication) function
```
```
10.4.7.10 Set the parameters of enabling NFC (Near-Field Communication) function
```
```
10.4.7.11 Get the configuration capability of enabling RF (Radio Frequency) card recognition
```
Hikvision co MMC
adil@hikvision.co.az
```
...
Response Message
```
10.4.7.12 Get the parameters of enabling RF (Radio Frequency) card recognition
```
```
10.4.7.13 Set the parameters of enabling RF (Radio Frequency) card recognition
```
Hikvision co MMC
adil@hikvision.co.az
```
...
<workMode>
10.4.7.14 Get the parameters of face recognition terminal
Hikvision co MMC
adil@hikvision.co.az
<workMode>
...
Request URL
10.4.7.15 Set the parameters of face recognition terminal
Hikvision co MMC
adil@hikvision.co.az
...
Request URL
10.4.7.16 Get configuration capability of face recognition terminal
Hikvision co MMC
adil@hikvision.co.az
...
None
Request Message
10.4.7.17 Set the parameters of M1 card encryption verification
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
10.4.7.18 Get the configuration parameters of M1 card encryption verification
10.4.7.19 Get the configuration capability of the M1 card encryption verification
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<M1CardEncryptCfg xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
wiegandID string --
Request Message
10.4.7.20 Get the capability of Wiegand parameters
10.4.7.21 Set Wiegand parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<WiegandCfg xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
None
Response Message
10.4.7.22 Get Wiegand parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<WiegandCfg xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
None
Response Message
10.4.7.23 Get configuration parameters of Wiegand rule
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
10.4.7.25 Get the configuration capability of the custom Wiegand rule
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<WiegandRuleCfg xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
10.4.8 Multi-Door Interlocking
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
```
10.4.8.1 Get the capability of configuring the multi-door interlocking
10.4.8.2 Get the multi-door interlocking configuration parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
```
Request URL
10.4.8.3 Set multi-door interlocking parameters
10.4.9 Multi-Factor Authentication
10.4.9.1 Get the capability of clearing group parameters
10.4.9.2 Clear group parameters
Hikvision co MMC
adil@hikvision.co.az
...
10.4.9.3 Get the group configuration parameters
10.4.9.4 Set the group parameters
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
Parameter Name Parameter Type Description
...
None
Response Message
10.4.9.5 Get the group configuration capability
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.9.6 Get the configuration parameters of multi-factor authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
doorID string --
Request Message
10.4.9.7 Set parameters of multi-factor authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.9.8 Get the configuration capability of multi-factor authentication modeHikvision co MMC
adil@hikvision.co.az
```
{
```
...
Query Parameter
10.4.10 Permission Schedules for Persons and Access Points
10.4.10.1 Set the holiday group parameters of the access permission control schedule
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
holidayGroupID string --
...
```
10.4.10.2 Get the holiday group configuration parameters of the access permission control schedule
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
holidayPlanID string --
Request Message
10.4.10.3 Get the holiday group configuration capability of the access permission control
10.4.10.4 Set the holiday schedule parameters of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.10.5 Get holiday schedule configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.10.6 Get the holiday schedule configuration capability of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.10.7 Get the schedule template configuration parameters of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Request Message
None
10.4.10.8 Set the schedule template parameters of the access permission control
10.4.10.9 Get the schedule template configuration capability of the access permission control
Hikvision co MMC
adil@hikvision.co.az
Response Message
```
...
None
Response Message
10.4.10.10 Get weekly schedule configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
Response Message
10.4.10.11 Set the week schedule parameters of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.10.12 Get the weekly schedule configuration capability of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
10.4.11 Person and Credential Synchronized Management
```
10.4.11.1 Get the capability of deleting person information (including linked cards, fingerprints, and faces)
```
and permissions
Hikvision co MMC
adil@hikvision.co.az
...
Request URL
```
10.4.11.2 Start deleting all person information (including linked cards, fingerprints, and faces) and
```
permissions by employee No.
10.4.12 Person and Credential Synchronized Management
```
10.4.12.1 Get the capability of deleting person information (including linked cards, fingerprints, and faces)
```
and permissions
Hikvision co MMC
adil@hikvision.co.az
...
Response Message
```
10.4.12.2 Start deleting all person information (including linked cards, fingerprints, and faces) and
```
permissions by employee No.
Hikvision co MMC
adil@hikvision.co.az
...
```
/*ro, req, object*/
10.4.13 Person and Credential Task Management
10.4.13.1 Delete a specified task of applying person data asynchronously
10.4.13.2 Get the capabilities of applying person data asynchronously
Hikvision co MMC
adil@hikvision.co.az
/*ro, req, object*/
"@min": 1,
...
Query Parameter
10.4.13.3 Get the status of all tasks of applying person data asynchronously
Hikvision co MMC
adil@hikvision.co.az
None
Request Message
...
10.4.14 Door Status Control
10.4.14.1 Get the capability set of remote door status control
10.5 Credentials Collection
10.5.1 Card Online Collection
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
10.5.1.1 Get the capability of collecting card information
10.5.2 Fingerprint Online Collection
10.5.2.1 Get the fingerprint collection capability
10.6 Zone Alarm
10.6.1 Alarm System Management
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
"isSptAlarmInCfg": true,
/*ro, opt, bool, whether it supports configuring alarm input parameters, desc:/ISAPI/SecurityCP/AlarmInCfg/<ID>?format=json*/
...
```
10.6.1.2 Get the configuration capability of security control panelHikvision co MMC
adil@hikvision.co.az
