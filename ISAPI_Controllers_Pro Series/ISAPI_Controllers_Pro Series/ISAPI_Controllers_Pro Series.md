...
Pro Series
```
DS-K2210, DS-K2220LX(P), DS-K2220X, DS-K2220X(P), DS-K2621X, DS-K2621X(P), DS-K2622X, DS-
```
```
K2622X(P), DS-K2624X, DS-K2624X(P), DS-K2M0016A
```
...
2.3.17 Person-Based Access Control
Hikvision co MMC
adil@hikvision.co.az
...
4. The device responds to SDP information.Hikvision co MMC
adil@hikvision.co.az
RTSP/1.0 200 OK
```
CSeq: 1
...
4.4.1.1 Arming with Subscription
Hikvision co MMC
adil@hikvision.co.az
...
```
```
filename=“ANPR.xml”; Content description. form-data is a piece of form data.
```
...
Hikvision co MMC
adil@hikvision.co.az1. Get the system capability: GET /ISAPI/System/capabilities. If the node isSupportMotionDetection is returned
...
7.1.3 Message Example
```
7.1.3.1 Motion Detection Event (Normal Mode)
```
```
7.1.3.2 Motion Detection Event (Expert Mode)
```
```
8 Access Control (General)
```
8.1 . Configure Multi-Door Interlocking
...
Calling Flow:
8.2 . Configure Parameters of Stand-Alone Anti-Passback
...
8.3 Access Point Unit Management
8.3.1 Introduction to the Function
8.3.2 API Calling Flow
8.3.2 Door Parameter Configuration
Hikvision co MMC
adil@hikvision.co.az
...
8.3.2.2 Pairing Parameters of Secure Door Control Unit
8.3.2.3 Switch Parameters of Secure Door Control Unit
8.3.2.4 Get Secure Door Control Unit Status
8.3.2.5 Parameters of Door Lock Status When Powered Off
8.3.2.6 Definition Rule of Magnetic Contact Status
8.3.2.7 Parameters Configuration of Unlocking Door
Hikvision co MMC
adil@hikvision.co.az
...
8.3.2.8 Get Lock Parameters
8.3.2.9 Access Devices Linked to Access Point
8.4 Arming Information
8.4.1 Introduction to the Function
Hikvision co MMC
adil@hikvision.co.az
type.
...
8.5 Authentication Schedule Management
8.5.1 Introduction to the Function
8.5.2 API Calling Flow
8.5.2.1 Card Reader's Authentication Schedule Configuration
Hikvision co MMC
adil@hikvision.co.az
...
8.5.2.2 Authentication Schedule Template Configuration
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follows:
...
8.5.2.3 Weekly Authentication Schedule Template Configuration
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follows:
...
8.5.2.4 Holiday Authentication Group Configuration
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follows:
...
8.5.2.5 Holiday Authentication Schedule Configuration
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follows:
...
8.6 Authentication via QR Code
8.6.1 Generate QR Code by Platform
8.6.1.1 Introduction to the Function
Hikvision co MMC
adil@hikvision.co.az
...
pictures.
8.6.2 Generate QR Code by Device
8.6.2.1 Introduction to the Function
...
8.6.3 Remarks
8.7 Card Management
8.7.1 Introduction to the Function
Hikvision co MMC
adil@hikvision.co.az
...
Note:
```
8.7.2 API Calling Flow
8.7.2.1 Check Whether the Device Supports Card Management
...
adding.
8.7.2.5 Card Information Editing
Hikvision co MMC
adil@hikvision.co.az
...
8.8 Configure Non-anti-passback Time Period
8.8.1 Introduction to the Function
8.8.2 API Calling Flow
Hikvision co MMC
adil@hikvision.co.az
ISAPI Protocol Calling Flow:
...
8.9 Door Control Schedule Management
8.9.1 Introduction to the Function
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follow:
8.9.2 API Calling Flow
8.9.2.1 Configure Door Control Schedule
...
8.9.2.2 Configure Door Control Schedule Template
...
8.9.2.3 Configure Door Control Weekly Schedule
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follow:
...
8.9.2.4 Configure Door Control Holiday Group
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follow:
...
8.9.2.5 Configure Door Control Holiday Schedule
Hikvision co MMC
adil@hikvision.co.az
The API calling flow is as follow:
...
8.10 Event and Card Linkage Parameters
8.10.1 Introduction to the Function
8.10.2 API Calling Flow
8.10.2.1 Configure Parameters of Event and Card Linkage
Hikvision co MMC
adil@hikvision.co.az
...
8.10.2.2 Search for Parameters of Event and Card Linkage
1. Get the functional capability of access control: GET /ISAPI/AccessControl/capabilities; if the node
...
format=json.
```
8.10.2.3 Delete Parameters of Specific Event and Card Linkage
1. Get the functional capability of access control: GET /ISAPI/AccessControl/capabilities; if the node
...
format=json.
```
8.10.2.4 Get List of Event and Card Linkage ID
...
format=json.
```
8.11 Face Picture Management
8.11.1 Introduction to the Function
Hikvision co MMC
adil@hikvision.co.az
...
Note:
```
8.11.2 API Calling Flow
8.11.2.1 Check Whether the Device Supports Face Picture Management
...
picture search.
8.11.2.3 Face Picture Applying
Hikvision co MMC
adil@hikvision.co.az
...
picture adding.
8.11.2.5 Face Picture Information Editing
Hikvision co MMC
adil@hikvision.co.az
...
8.11.2.6 Face Picture Deleting
...
8.11.2.7 Face Picture Collecting
...
8.12 Fingerprint Management
8.12.1 Introduction to the Function
8.12.2 API Calling Flow
8.12.2.1 Check Whether the Device Supports Fingerprint Management
Hikvision co MMC
adil@hikvision.co.az
...
8.12.2.5 Fingerprint Information Editing
...
8.12.2.7 Fingerprint Collecting
...
Note:
```
8.13.1 Introduction to the Function
8.13.2 API Calling Flow
8.13.2.1 Check Whether the Device Supports Iris Data Management
...
8.14 Multi-Factor Authentication
8.14.1 Introduction to the Function
8.14.2 API Calling Flow
8.14.2.1 Group Parameter Configuration
...
8.14.2.3 Multi-Factor Authentication Mode Configuration
...
8.15 Person and Credential Management
8.15.1 Introduction to the Function
8.16 Person Management
8.16.1 Introduction to the Function
8.16.2 API Calling Flow
8.16.2.1 Check Whether the Device Supports Person Management
Hikvision co MMC
adil@hikvision.co.az
...
person adding.
8.16.2.5 Person Information Editing
Hikvision co MMC
adil@hikvision.co.az
...
```
8.17 Reset Anti-Passback Rule (Additional Function)
```
8.17.1 Introduction to the Function
...
8.18 Schedules Management of Persons' Access Permission
8.18.1 Introduction to the Function
Hikvision co MMC
adil@hikvision.co.az
...
8.18.2 API Calling Flow
8.18.2.1 Schedule Template of Persons' Access Permission
Hikvision co MMC
adil@hikvision.co.az
Calling Flow:
...
8.18.2.2 Weekly Schedule of Persons' Access Permission
Hikvision co MMC
adil@hikvision.co.az
Calling Flow:
...
8.18.2.3 Holiday Groups of Persons' Access Permission
Hikvision co MMC
adil@hikvision.co.az
Calling Flow:
...
8.18.2.4 Holiday Schedule of Persons' Access Permission
Hikvision co MMC
adil@hikvision.co.az
Calling Flow:
...
8.19 Search for QR Code Scanning Event
...
8.20 Store and Search for Access Control Event
8.20.1 Introduction to the Function
8.20.2 API Calling Flow
8.20.2.1 Configure Storage Parameters of Access Control Events
1. Call the functional capability of access control: GET /ISAPI/AccessControl/capabilities; if the node
...
format=json.
```
8.20.2.2 Search for Access Control Events
1. Call the functional capability of access control: GET /ISAPI/AccessControl/capabilities; if the node
...
8.20.2.3 Get Total Number of Access Control Events
1. Call the functional capability of access control: GET /ISAPI/AccessControl/capabilities; if the node
...
```
9 Security Control Device (General)
```
Hikvision co MMC
adil@hikvision.co.az
...
9.1 Advanced Configuration for Telephone Notification
Hikvision co MMC
adil@hikvision.co.az
control panel.
...
9.2.2 ARC Parameter Configuration
Hikvision co MMC
adil@hikvision.co.az
...
9.3 Capture Pictures of Specific Zones
9.3.1 Upload Pictures by Pircams
9.3.2 Get Captured Pictures in Asynchronous Mode
Hikvision co MMC
adil@hikvision.co.az
API Calling Steps:
...
API Calling Flow:
9.4 Custom Text Message Management
Hikvision co MMC
adil@hikvision.co.az
Hikvision co MMC
adil@hikvision.co.az
...
parameters.
API Calling Flow:
9.5 Email Notification Management
Hikvision co MMC
adil@hikvision.co.az
API Calling Steps:
...
```
API Calling Flow:
9.6 Network Center Parameter Configuration
Hikvision co MMC
adil@hikvision.co.az
API Calling Steps:
1. Get the configuration capability of the security control panel to check whether the device supports configuring
...
```
10.1.1 Peripheral No.
10.2 Topology of Detectors and Peripherals
10.2.1 Topology of Detectors
...
Query Parameter
10.2.2 Topology of Peripherals
...
11 API Reference
```
11.1 Device Management (General)
```
11.1.1 Event Subscription Management
11.1.1.1 Get alarm/event subscription parameters
Hikvision co MMC
adil@hikvision.co.az
Parameter
Name
...
Type Description
```
deployID enum 0 (device local arming), 1 (real-time arming by platform). If the deployID is absent, itdefaults to device local arming.
```
Request Message
<?xml version="1.0" encoding="UTF-8"?>
<SubscribeEvent xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
```
<heartbeat>
11.1.1.2 Subscribe to events/alarms in arming mode
Hikvision co MMC
adil@hikvision.co.az
<!--opt, int, heartbeat interval time, range:[1,180], unit:s-->1
</heartbeat>
<channelMode>
```
<!--opt, enum, channel subscription mode, subType:string, desc:"all" (subscribe to all channels), "list" (subscribe to channels according to channel
```
```
list)-->list
```
...
<eventMode>
```
<!--req, enum, event subscription mode, subType:string, desc:"all" (subscribe to all alarms/events), "list" (subscribe to specified alarms/events)--
```
>list
</eventMode>
<EventList>
...
<Event>
<!--opt, object, subscription of a specified alarm/event-->
<type>
```
<!--req, enum, event type, subType:string, desc:refer to event type list (eventType): "ADAS"(advanced driving assistance system), "ADASAlarm"
```
```
...
```
```
"videoloss", "violationAlarm", "violentMotion"(violent motion detection), "yardTarry"(playground overstay detection), "CertificateCaptureEvent"(person ID
```
```
capture comparison event), "UncertificateCompareEvent", "ConsumptionAndTransactionRecordEvent", "ConsumptionEvent", "TFS" (traffic enforcement event),
```
```
"TransactionRecordEvent", "SetMealQuery"(searching consumption set meals), "ConsumptionStatusQuery"(searching the consumption status),
```
```
"certificateRevocation" (certificate expiry), "humanBodyComparison" (human body comparison), "regionTargetNumberCounting" (regional target statistics)--
```
>mixedTargetDetection
</type>
<minorAlarm>
<!--opt, string, minor alarm type, desc:this node is required when type is "AccessControllerEvent"-->0x400,0x401,0x402,0x403
</minorAlarm>
<minorException>
<!--opt, string, minor exception type, desc:this node is required when type is "AccessControllerEvent"-->0x400,0x401,0x402,0x403
</minorException>
<minorOperation>
<!--opt, string, minor operation type, desc:this node is required when type is "AccessControllerEvent"-->0x400,0x401,0x402,0x403
</minorOperation>
<minorEvent>
<!--opt, string, minor event type, desc:this node is required when type is "AccessControllerEvent"-->0x01,0x02,0x03,0x04
</minorEvent>
<pictureURLType>
```
...
<channels>
<!--opt, string, channel information linked to event, desc:it supports multiple channels, which are separated by commas-->1,2,3,4
</channels>
<ConferenceIDList>
...
<minorType>
<!--opt, enum, event sub-type, subType:string,
```
...
check-in.
3. When this node is not applied, it indicates to subscribe to both success and failure of the event. "success", "failed"-->success
</minorType>
<childDevIDList>
```
<!--opt, array, (sub-device local identifier) device serial No. list, subType:object, desc:arm sub device (sensor)-->
```
<childDevID>
```
<!--opt, string, (sub-device local identifier) device serial No. list, range:[1,128], desc:arm sub device (sensor)-->test
```
</childDevID>
</childDevIDList>
Hikvision co MMC
...
<radarChannels>
<!--opt, string, arm by radar channel, desc:it supports multiple channels, which are separated by commas-->1,2,3,4
</radarChannels>
<audioChannels>
<!--opt, string, arm by audio channel, desc:it supports multiple channels, which are separated by commas-->1,2,3,4
</audioChannels>
<childDeviceFormat>
<!--opt, bool, upload by sub device, desc:upload by sub device-->true
</childDeviceFormat>
<unitChannels>
<!--opt, string, arm by optical fiber channel, desc:it supports multiple channels, which are separated by commas-->1,2,3,4
</unitChannels>
</Event>
</EventList>
<channels>
<!--opt, string, unified arming of all channels, desc:if this node is applied, <channels> in <Event> will be invalid. If the value of <channelMode> is
all, this node should not be applied-->1,2,3,4
</channels>
<pictureURLType>
```
<!--opt, enum, format unified configuration of alarm pictures, subType:string, desc:"binary", "localURL" (device local URL), "cloudStorageURL" (cloud
```
```
storage URL). The node indicates the upload mode of all event pictures. If the node is applied, <pictureURLType> of <Event> will be invalid. If the node is
```
not applied, the pictures are uploaded in the default mode. The default data type of uploaded pictures for front-end devices is binary, and for back-end
...
<interval>
```
<!--opt, int, the lifecycle of arming GUID, range:[1,300], unit:s, desc:the default value is 5 (unit: minute). Within the interval, if the client
```
software does not reconnect to the device, a new GUID will be generated by the device-->300
</interval>
<StatusSub>
<!--opt, object, status information-->
<all>
<!--opt, bool, whether to subscribe to the changing status of all channels, HDDs, and capability sets-->false
</all>
<channel>
<!--opt, bool, status of subscribed channels, desc:it is not required if the value of <all> is true-->true
</channel>
<hd>
<!--opt, bool, subscribe to HDD status, desc:it is not required if the value of <all> is true-->true
</hd>
<capability>
<!--opt, bool, subscribe to changing status of capability set, desc:it is not required if the value of <all> is true-->true
</capability>
<ChanStatus>
...
<subscribeEventID>
<!--opt, string, custom subscription ID. The platform should guarantee the uniqueness of ID when applying it. The automatic network replenishment of
```
events will be enabled by default after the node is applied, range:[1,128], desc:when an event/alarm is subscribed (related URL: POST
```
```
/ISAPI/Event/notification/subscribeEvent), if this node is applied, it can be used to replace <id> (subscription ID) returned in message
```
...
<!--opt, object-->
<EventTypeList>
<!--opt, array, event list, subType:object, desc:event list-->
<eventType>
<!--req, enum, event type, subType:string, desc:event type-->AIOP_Video
</eventType>
<uploadPicEnabled>
<!--opt, bool, whether to upload the event picture, desc:whether to upload the event picture-->false
</uploadPicEnabled>
</EventTypeList>
<EventList>
<!--req, array, event list, subType:object, range:[0,200]-->
<Event>
<!--opt, object, a single event-->
<eventType>
<!--req, enum, event type, subType:string, desc:event type-->AIOP_Video
</eventType>
<uploadPicEnabled>
...
<radarChannels>
<!--opt, string, batch arming by radar channel, desc:if this field is applied, radarChannels in Event will not take effect. To arm all radar channels,
this field should not be applied. To arm a part of channels, you can list the channel numbers. Multiple channels are separated by commas-->1,2,3,4
</radarChannels>
<qosLevel>
<!--ro, opt, object, subscription events qos level, desc:subscription events qos level-->
<defaultLevel>
<!--ro, req, enum, default qos level, subType:int, desc:default qos level-->0
</defaultLevel>
<eventList>
...
<level>
<!--ro, req, enum, qos level of this event type, subType:int, desc:qos level of this event type-->1
</level>
<channels>
```
...
<unitChannels>
<!--opt, string, batch arming by optical fiber channel, desc:if this field is applied, unitChannels in Event will not take effect. To arm all fiber
channels, this node should not be applied. To arm a part of channels, you can list the channel numbers. Multiple channels are separated by commas-->1,2,3,4
</unitChannels>
<retransmissionID>
```
<!--opt, string, ANR arming ID, range:[1,64], desc:UUID (an unique ID generated by the client). If this field is applied during establishing the event
```
```
arming/subscription connection, the ANR of subscribed event will be enabled by default; when the network reconnected, the platform can apply the same UUID
```
to tell the device to re-upload saved event information during network disconnection period-->123456
</retransmissionID>
<retransmissionTimeoutDuration>
<!--opt, int, data transmission timeout for ANR, range:[1,168], unit:h, desc:data transmission timeout for ANR-->1
</retransmissionTimeoutDuration>
<subscribeWithoutPic>
<!--opt, bool-->false
</subscribeWithoutPic>
...
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<SubscribeEventResponse xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
```
<!--ro, req, object, subscription result, attr:version{req, string, protocolVersion}-->
```
<id>
<!--ro, req, int, subscription ID, which is returned by the device. The device should guarantee the uniqueness of ID, range:[1,64]-->1
</id>
<FailedEventList>
<!--ro, opt, array, list of events, of which subscription failed, subType:object, desc:when subscription of one or multiple events failed, this node
should be returned. The client software can check if all events are subscribed to by checking whether the node <FailedEventList> is returned-->
<Event>
<!--ro, opt, object, event, of which the subscription failed. The event/alarm type name can be obtained via the capability set-->
<type>
<!--ro, req, enum, event type, subType:string, desc:event type-->ADAS
</type>
<minorAlarm>
<!--ro, opt, string, minor alarm type, desc:refer to the macro definition of uploaded events. "IDCardInfoEvent" is required when the type of event
is "AccessControllerEvent"-->0x400,0x401,0x402,0x403
...
<pictureURLType>
```
<!--ro, opt, enum, alarm picture format, subType:string, desc:"binary", "localURL" (local URL), "cloudStorageURL" (cloud storage URL).-->binary
```
</pictureURLType>
<channels>
<!--ro, opt, string, channel No., desc:it supports multiple channels, which are separated by commas-->1,2,3,4
</channels>
<subStatusCode>
<!--ro, req, enum, error code, subType:string, desc:refer to the error code dictionary-->activeNumMax
</subStatusCode>
<minorType>
<!--ro, opt, enum, event sub-type, subType:string,
```
desc:1. It is valid when the value of <type> is "CertificateCaptureEvent" or "ncertificateCompareEvent".
```
2. There is the requirement to only subscribe to the failure or success of an event, for example, security inspection at the railway station and the hotel
check-in.
3. When this node is not returned, it indicates to subscribe to both success and failure of the event. "success", "failed"-->success
</minorType>
</Event>
</FailedEventList>
</SubscribeEventResponse>
...
None
Response Message
11.1.1.3 Get the alarm/event subscription capability
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
EventType:heartBeat
```
11.1.1.4 Event subscription heartbeat
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
<newVersion max="64">
11.1.2 Online Upgrading Management
11.1.2.1 Get the device online upgrade capability
Hikvision co MMC
adil@hikvision.co.az
<newVersion max="64">
```
...
None
Response Message
11.1.2.2 Get the online upgrade package downloading progress
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.1.2.3 Start to download upgrade package to device
11.1.2.4 Get the online upgrade progress of device
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
Request URL
GET /ISAPI/System/onlineUpgrade/status/task?format=json
Query Parameter
None
Request Message
None
Response Message
11.1.2.5 Get all of the online upgrade task progress
Hikvision co MMC
adil@hikvision.co.az
```
{
```
"onlineUpgradeStatusList": [
/*ro, req, array, online upgrade status list, subType:object*/
```
{
```
"status": "notUpgrade",
```
/*ro, req, enum, upgrade status, subType:string, desc:“notUpgrade”, “upgrading”, “successful” (upgrading succeeded), “languageMismatch”,
```
```
“writeFlashError”, “packageTypeMismatch”, “packageVersionMismatch”, “netUnreachable” (network disconnected), “unknownError”, “waitManualTrigger”*/
```
"percent": 0,
/*ro, req, int, upgrade progress percentage, range:[0,100]*/
"taskID": "test",
/*ro, opt, string, task ID, range:[1,64], desc:generated when creating upgrade tasks, related URI: /ISAPI/System/onlineUpgrade/task?
```
format=json*/
```
"moudleType": "stationHostAndScreen",
```
/*ro, opt, enum, component type, subType:string, desc:"stationHostAndScreen" (dock station upgrade package, including host and screen),
```
```
"stationHost" (dock station host), "spellingControllerMainAndSubBoard" (main board and sub board of splicing controller), "spellingControllerMainBoard"
```
```
(main board of splicing controller), "spellingControllerSubBoard" (sub board of splicing controller), "bodyCamera" (body camera)*/
```
"packageName": "ZCS-H1_target_V1.0.1_build201123.zip",
/*ro, opt, string, upgrade package name, range:[1,128]*/
"taskTime": "1970-01-01T00:00:00+08:00",
/*ro, opt, datetime, applying upgrade task time*/
```
"taskParam": {
```
/*ro, opt, object, task parameters, desc:corresponding URL: POST /ISAPI/System/onlineUpgrade/task?format=json*/
"URL": "test",
/*ro, req, string, upgrade package address, range:[1,1024]*/
"delayedUpgrade": true,
```
/*ro, opt, bool, whether to delay the upgrade, desc:if this node is not applied, it indicates that the upgrade will be started immediately;
```
if this node is applied and the value is false, it indicates that the upgrade will be delayed*/
"delayedUpgradeDateTime": "1970-01-01T00:00:00+08:00"
```
/*ro, opt, datetime, time of starting the delayed upgrade, dep:or,{$.PackageDescription.delayedUpgrade,eq,false}*/
```
```
}
```
...
None
Response Message
11.1.3 Serial Port Management
11.1.3.1 Get the serial port capability of the device
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
11.1.3.2 Get the capability of a specific serial port
11.1.3.3 Set the parameters of a specific serial port supported by the device
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
Parameter Name Parameter Type Description
...
<childDevID>&deviceIndex=<deviceIndex>
Query Parameter
11.1.3.4 Get the parameters of a specific port supported by the device
Hikvision co MMC
adil@hikvision.co.az
Parameter
Name
...
None
Response Message
11.1.3.5 Get the list of serial ports supported by the device
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
<!--ro, opt, bool, whether it supports configuring network camera parameters on the LAN, desc:related URI:
...
Request Message
None
11.1.4 System Maintenance
11.1.4.1 Get the storage capability of the device
11.1.4.2 Get the system security capability
Hikvision co MMC
adil@hikvision.co.az
Response Message
<?xml version="1.0" encoding="UTF-8"?>
...
```
phone number, which is similar with password reset via email. The phone number can be changed on the website or local UI of the device _x000D_
...
<isSupportWired>
<!--ro, opt, bool, whether it supports wired network-->true
</isSupportWired>
<isSupportPPPoE>
```
...
11.1.4.3 Get device system capability
Hikvision co MMC
adil@hikvision.co.az
</SnmpCap>
<isSupportSSH>
...
<DeployCap>
<!--ro, opt, object, the capability of arming non-subscription events-->
<deployID opt="1,2">
```
<!--ro, opt, enum, arming type, subType:int, attr:opt{req, string}, desc:0 (client arming); 1 (real-time arming)-->0
```
</deployID>
</DeployCap>
...
<isSupportSearchRebootBatchChildStatus>
<!--ro, opt, bool, whether it supports searching for the status of batch rebooting sub-devices, desc:POST /ISAPI/System/SearchRebootBatchChildStatus?
```
format=json-->true
```
</isSupportSearchRebootBatchChildStatus>
<isSupportRebootBatchChild>
<!--ro, opt, bool, whether it supports batch restarting sub-devices, desc:PUT /ISAPI/System/RebootBatchChild?format=json-->true
</isSupportRebootBatchChild>
</DeviceCap>
Request URL
GET /ISAPI/System/DeviceLanguage
11.1.4.4 Get the languages supported by the device
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
None
...
None
Request Message
11.1.4.5 Set device language parameters
11.1.4.6 Get the capability of configuring the device language
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
None
Request Message
11.1.4.7 Set restoring devices' default parameters
11.1.4.8 Get the network service capability
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
<isSupportEZVIZQRCode>
<!--ro, opt, bool, whether the device supports getting the EZ QR code, desc:/ISAPI/System/Network/EZVIZ/QRCode?format=json-->true
</isSupportEZVIZQRCode>
<isSupportEZVIZUnbind>
<!--ro, opt, bool, whether the device supports unbinding from EZ, desc:/ISAPI/System/Network/EZVIZ/unbind?format=json-->true
</isSupportEZVIZUnbind>
...
11.1.5 Upgrade Management
11.1.5.1 Get the capabilities of peripheral module upgrade
Hikvision co MMC
adil@hikvision.co.az
</securityModuleNo>
...
Request Message
None
11.1.5.3 Get the device upgrading status and progress
11.1.6 Time Management
11.1.6.1 Get device time synchronization management parameters
Hikvision co MMC
adil@hikvision.co.az
Response Message
<?xml version="1.0" encoding="UTF-8"?>
...
None
Request Message
11.1.6.2 Set device time synchronization management parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<Time xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
11.1.6.3 Get the capability of device time synchronization management
Hikvision co MMC
adil@hikvision.co.az
pe/Chisinau,Europe/Copenhagen,Europe/Dublin,Europe/Gibraltar,Europe/Helsinki,Europe/Isle_of_Man,Europe/Istanbul,Europe/Kiev,Europe/Lisbon,Europe/London,Euro
...
Binary Data
Response Message
11.1.6.4 Set device local time
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<ResponseStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
Response Message
11.1.6.5 Set parameters of all NTP servers
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<ResponseStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
NTPServerID string --
```
11.1.6.6 Get the parameters of a specific NTP (Network Time Protocol) server
```
11.1.6.7 Set the parameters of a NTP server
Hikvision co MMC
adil@hikvision.co.az
Request Message
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
11.1.6.8 Get the parameters of a NTP serverHikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<NTPServer xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
```
...
Response Message
```
11.1.6.9 Get the configuration capability of parameters of a specific NTP (Network Time Protocol) server
```
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
11.1.6.10 Get time zone
11.1.6.11 Set time zone
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<ResponseStatus xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
11.2.1 HTTP Listening Management
```
11.2.1.1 Delete receiving server(s)
```
11.2.1.2 Delete a HTTP listening server
Hikvision co MMC
adil@hikvision.co.az
Parameter
Name
...
<ipAddress>
11.2.1.3 Set the parameters of a listening host
Hikvision co MMC
adil@hikvision.co.az
<ipAddress>
```
...
```
11.2.1.4 Get the parameters of a listening host
Hikvision co MMC
adil@hikvision.co.az
```
<!--ro, opt, string, host name, dep:and,{$.HttpHostNotification.addressingFormatType,eq,hostName}-->test
...
None
Response Message
11.2.1.5 Get the capabilities of listening hosts parameters
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
None
```
11.2.1.6 Set IP address of receiving server(s)
```
Hikvision co MMC
adil@hikvision.co.az
Request Message
...
None
Request Message
11.2.1.7 Get parameters of all listening hosts
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
11.3.1.1 Get arming schedule of motion detection of all channels
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
<dayOfWeek>
```
<!--ro, opt, enum, days of the week, subType:int, desc:1 (Monday), 2 (Tuesday), 3 (Wednesday), 4 (Thursday), 5 (Friday), 6 (Saturday), 7 (Sunday)-
```
->1
</dayOfWeek>
<TimeRange>
...
None
Request Message
11.3.1.2 Set arming schedule of motion detection of all channels
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<MotionDetectionScheduleList xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
<eventType>
```
<!--opt, enum, event type, subType:string, desc:"VMD” (motion detection)-->VMD
```
</eventType>
<inputIOPortID>
<!--opt, string-->test
...
```
11.4 Access Control (General)
```
11.4.1 Access Control Event Management
11.4.1.1 Get the capability of searching for access control events
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
```
Request URL
11.4.1.2 Get the configuration capability of storing access control events
11.4.1.3 Get the storage parameters of access control events
Hikvision co MMC
adil@hikvision.co.az
...
None
Request Message
11.4.1.4 Set storage parameters of access control events
11.4.1.5 Search for access control events
Hikvision co MMC
adil@hikvision.co.az
```
{
...
11.4.1.6 Get the capability of getting total number of access control events by specific conditions
Hikvision co MMC
adil@hikvision.co.az
/*ro, req, object, major alarm type*/
"@opt": "0,1,2,3,5"
...
```
Request URL
11.4.1.7 Get the total number of access control events by specific conditions
Hikvision co MMC
adil@hikvision.co.az
...
Query Parameter
11.4.1.8 Get the capability of clearing event and card linkage parameters
11.4.1.9 Clear event card linkage configurations
Hikvision co MMC
adil@hikvision.co.az
None
Request Message
...
Request URL
11.4.1.10 Getting arming information
11.4.1.11 Getting arming information capability
Hikvision co MMC
adil@hikvision.co.az
...
```
11.4.1.12 Set the event card linkage parameters
Hikvision co MMC
adil@hikvision.co.az
Response Message
```
...
None
Response Message
11.4.1.13 Get the event and card linkage configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
alarm input ID*/
"eventSourceIDList": [1, 2, 3],
```
/*ro, opt, array, subType:int, dep:or,{$.EventCardLinkageCfg.proMode,eq,event},{$.EventCardLinkageCfg.proMode,eq,card},
```
```
{$.EventCardLinkageCfg.proMode,eq,employee}*/
```
"doorNameList": [
/*ro, opt, array, subType:object*/
```
...
```
"doorName": "dvyed"
/*ro, req, string, range:[0,32]*/
```
}
```
],
"alarmoutTriggerRuleEnabled": true,
```
/*ro, opt, bool, dep:or,{$.EventCardLinkageCfg.proMode,eq,card},{$.EventCardLinkageCfg.proMode,eq,employee}*/
```
"openDoor": [1, 3, 5],
```
...
```
11.4.1.14 Get the configuration capability of the event and card linkage
Hikvision co MMC
adil@hikvision.co.az
```
},
...
None
Response Message
11.4.1.15 Get the capability of the list of event and card linkage ID
11.4.1.16 Get the list of event and card linkage ID
Hikvision co MMC
adil@hikvision.co.az
```
{
...
/*ro, opt, object*/
"id": [1, 2, 3]
/*ro, req, array, list of configured event and card linkage ID, subType:int, desc:[1, 2, 3] indicates that the device is configured with event
linkage 1, 2, and 3*/
```
}
```
...
```
Request URL
11.4.1.17 Get the configuration capability of event optimization
11.4.1.18 Set the event optimization parameters
11.4.1.19 Get the event optimization configuration parameters
Hikvision co MMC
adil@hikvision.co.az
...
```
11.4.1.20 Access control event
Hikvision co MMC
adil@hikvision.co.az
```
/*ro, opt, enum, card reader type, subType:int, desc:1 (IC card reader), 2 (ID card reader), 3 (QR code scanner), 4 (fingerprint module)*/
...
11.4.2 Access Point Status Schedule
11.4.2.1 Get the capability of clearing access control schedule configuration.
11.4.2.2 Clear access control schedule configuration parametersHikvision co MMC
adil@hikvision.co.az
```
{
```
...
```
Response Message
11.4.2.3 Set holiday group parameters of door control schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.2.4 Get the holiday group configuration parameters of the door control schedule
11.4.2.5 Get the configuration capability of door status parameters of holiday group
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
11.4.2.6 Get the configuration parameters of the door control holiday schedule
11.4.2.7 Set parameters of door control holiday schedule
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
None
Response Message
11.4.2.8 Get the configuration capability of the door control holiday schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
Request URL
11.4.2.9 Get the configuration parameters of the door control schedule
11.4.2.10 Set parameters of door control schedule
Hikvision co MMC
adil@hikvision.co.az
...
Query Parameter
11.4.2.11 Get the configuration capability of the door control schedule
11.4.2.12 Set parameters of door control schedule template
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
...
```
11.4.2.13 Get parameters of door control schedule template
11.4.2.14 Get the configuration capability of the door control schedule template
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
weekPlanID string --
Request Message
11.4.2.15 Set parameters of door control weekly schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.2.16 Get the configuration parameters of the door control week schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.2.17 Get the configuration capability of the door control week schedule
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.3 Access Control Device Management
11.4.3.1 Get the configuration capability of the access controller
Hikvision co MMC
adil@hikvision.co.az
```
{
...
11.4.3.2 Set the parameters of the access controller
11.4.3.3 Get the configuration parameters of the access controller
Hikvision co MMC
adil@hikvision.co.az
"showCapPic": true,
...
11.4.3.4 Get the capability of getting the working status of
Hikvision co MMC
adil@hikvision.co.az
"@max": 1
/*ro, opt, int, storage battery power voltage*/
...
None
Response Message
11.4.3.5 Get the working status of the access controller
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Query Parameter
11.4.4 Access Control Module Management
11.4.4.1 Get the door configuration capability
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
...
```
11.4.4.2 Set the door (floor) parameters
```
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
...
```
11.4.4.3 Get the door (floor) configuration parameters
```
Hikvision co MMC
adil@hikvision.co.az
<!--ro, opt, int, alarm time of magnetic contact detection timeout, range:[0,255], unit:s, desc:0 refers to not triggering alarm-->1
...
```
11.4.5 Anti-Passback
11.4.5.1 Get the anti-passing back configuration capability
11.4.5.2 Get the parameters of anti-passback configuration
11.4.5.3 Set the anti-passing back parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Query Parameter
11.4.5.4 Get the anti-passing back configuration parameters of a specified card reader
11.4.5.5 Set anti-passing back parameters of a card reader
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
cardReaderID string --
...
```
Request URL
11.4.5.6 Get the configuration capability of anti-passing back parameters of card readers
11.4.5.7 Get the capability of clearing anti-passback records
Hikvision co MMC
adil@hikvision.co.az
...
```
11.4.5.8 Clear anti-passback records in the device
11.4.5.9 Get the capability of clearing anti-passback parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Query Parameter
11.4.5.10 Clear anti-passing back parameters
11.4.6 Authentication Schedule Management
11.4.6.1 Set control schedule parameters of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
cardReaderID string --
...
None
Response Message
11.4.6.2 Get the control schedule configuration parameters of the card reader authentication mode
11.4.6.3 Get the control schedule configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Query Parameter
11.4.6.4 Get the holiday group configuration parameters of the control schedule of the card reader
authentication mode
11.4.6.5 Set holiday group parameters of control schedule of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
holidayGroupID string --
...
```
11.4.6.6 Get the holiday group configuration capability of the control schedule of the card reader
authentication mode
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
holidayPlanID string --
Request Message
11.4.6.7 Get holiday schedule parameters of the card reader authentication mode
11.4.6.8 Set holiday schedule parameters of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.6.9 Get the holiday schedule configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Request Message
None
11.4.6.10 Get the schedule template parameters of card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
Response Message
```
...
None
Request Message
11.4.6.11 Set the schedule template parameters of the card reader authentication mode
11.4.6.12 Get the schedule template configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
None
Response Message
11.4.6.13 Get the week schedule configuration parameters of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
weekPlanID string Weekly schedule No., which starts from 1, and the maximum value supported by thedevice is obtained from the capability set.
Request Message
11.4.6.14 Set the week schedule parameters of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.6.15 Get the weekly schedule configuration capability of the card reader authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
cardReaderID string --
Request Message
11.4.7 Credential Recognition Module Management
11.4.7.1 Set the card reader parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
cardReaderID string --
Request Message
11.4.7.2 Get the card reader configuration parameters
Hikvision co MMC
adil@hikvision.co.az
None
Response Message
...
None
Response Message
11.4.7.3 Get the configuration capability of the card reader
Hikvision co MMC
adil@hikvision.co.az
```
{
...
"isSupportOfflineSendPermission": true,
```
/*ro, opt, bool, whether to enable applying permissions offline on the card reader, desc:AIOT (Artificial Intelligence & Internet of Things)
```
supports apply person's permissions when the card reader is offline*/
```
"buzzerTime": {
```
```
...
"cardAndPw,card,cardOrPw,fp,fpAndPw,fpOrCard,fpAndCard,fpAndCardAndPw,faceOrFpOrCardOrPw,faceAndFp,faceAndPw,faceAndCard,face,employeeNoAndPw,fpOrPw,employe
eNoAndFp,employeeNoAndFpAndPw,faceAndFpAndCard,faceAndPwAndFp,employeeNoAndFace,faceOrfaceAndCard,fpOrface,cardOrfaceOrPw,cardOrFace,cardOrFaceOrFp,faceOrPw
,employeeNoAndFaceAndPw,cardOrFaceOrFaceAndCard,iris,faceOrFpOrCardOrPwOrIris,faceOrCardOrPwOrIris,cardOrFpOrFaceOrIris,fpOrFaceOrIrisOrPw,cardOrFpOrIrisOrP
w,cardOrIrisOrPw,cardAndIris,fpAndIris,faceAndIris,irisAndPw,cardAndIrisAndPw,faceAndIrisAndPw,cardAndFaceAndIris,Pw,cardOrFpOrPw,faceOrFpOrPw,cardAndVp,fac
eAndVp,fpAndVp,pwAndVp,remoteControl,remoteControlAndFace,remoteControlOrFace,remoteControlOrCard,remoteControlOrFp,remoteControlOrPw,remoteControlOrCardOrF
p,remoteControlOrCardOrFace,remoteControlOrCardOrPw,remoteControlOrFpOrFace,remoteControlOrFpOrPw,remoteControlOrFaceOrPw,remoteControlOrCardOrFpOrFace,remo
teControlOrCardOrFpOrPw,remoteControlOrFpOrFaceOrPw,remoteControlOrCardOrFaceOrPw,remoteControlOrCardOrFpOrFaceOrPw,faceOrPPPV,cardOrPPPV,pwOrPPPV,faceOrCar
dOrPPPV,faceOrPwOrPPPV,faceOrCardOrPwOrPPPV,faceAndPPPV,cardAndPPPV,pwAndPPPV,faceAndCardAndPPPV,faceAndCardAndPwAndPPPV,PPPV,faceAndCardAndPw"
/*ro, req, string, options*/
```
},
```
...
```
/*ro, opt, object*/
"@opt": ["all", "byCardReader", "byRegion"]
/*ro, req, array, subType:string*/
```
},
```
```
"searchCardReaderParams": {
```
/*ro, opt, object*/
```
"searchID": {
...
None
Response Message
11.4.7.4 Get the capability of configuring card No. authentication rule
11.4.7.5 Get the switching progress and configuration result of card No. authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
11.4.7.6 Get the parameters of card No. authentication mode
11.4.7.7 Set the parameters of card No. authentication mode
```
11.4.7.8 Get the configuration capability of enabling NFC (Near-Field Communication) function
```
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Response Message
```
11.4.7.9 Get the parameters of enabling NFC (Near-Field Communication) function
```
```
11.4.7.10 Set the parameters of enabling NFC (Near-Field Communication) function
```
Hikvision co MMC
adil@hikvision.co.az
```
...
```
```
11.4.7.11 Get the configuration capability of enabling RF (Radio Frequency) card recognition
```
```
11.4.7.12 Get the parameters of enabling RF (Radio Frequency) card recognition
```
```
11.4.7.13 Set the parameters of enabling RF (Radio Frequency) card recognition
```
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
11.4.7.14 Get the parameters of face recognition terminal
Hikvision co MMC
adil@hikvision.co.az
</blackListCheck>
<idCardCheckCenter>
...
11.4.7.15 Set the parameters of face recognition terminal
Hikvision co MMC
adil@hikvision.co.az
<workMode>
...
11.4.7.16 Get configuration capability of face recognition terminal
Hikvision co MMC
adil@hikvision.co.az
<serialOutput opt="enable,disable">
```
...
11.4.7.17 Set the parameters of M1 card encryption verification
11.4.7.18 Get the configuration parameters of M1 card encryption verification
11.4.7.19 Get the configuration capability of the M1 card encryption verification
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
Request URL
11.4.7.20 Get the capability of Wiegand parameters
11.4.7.21 Set Wiegand parameters
Hikvision co MMC
adil@hikvision.co.az
...
Request Message
None
11.4.7.22 Get Wiegand parameters
Hikvision co MMC
adil@hikvision.co.az
Response Message
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
11.4.7.23 Get configuration parameters of Wiegand rule
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
```
<name>
<!--ro, req, string, Wiegand name-->test
</name>
<CustomerCardIn>
<!--ro, opt, object-->
<totalLength>
<!--ro, req, int, total Wiegand length, desc:when this node is set to 0, the custom Wiegand rule settings will be cleared-->1
</totalLength>
<checkMethod>
```
<!--ro, req, enum, parity mode, subType:string, desc:parityCheck, xorCheck, noCheck;-->parityCheck
```
</checkMethod>
<ParityCheck>
<!--ro, opt, object, configuration rule of odd-even parity, desc:this node is valid when <checkMethod> is "parityCheck”-->
<oddBeginBit>
<!--ro, opt, int, start bit of odd parity-->1
</oddBeginBit>
<oddLength>
<!--ro, opt, int, odd parity length-->1
</oddLength>
<evenBeginBit>
<!--ro, opt, int, start bit of even parity-->1
</evenBeginBit>
<evenLength>
<!--ro, opt, int, even parity length-->1
</evenLength>
</ParityCheck>
<XorCheck>
<!--ro, opt, object, configuration rule of XOR parity, desc:this node is valid when <checkMethod> is "xorCheck”-->
<xorBeginBit>
<!--ro, opt, int, start bit of XOR parity-->1
</xorBeginBit>
<xorPerLength>
<!--ro, opt, int, length of each XOR parity group-->1
</xorPerLength>
<xorTotalLength>
<!--ro, opt, int, total length of XOR parity data-->1
</xorTotalLength>
</XorCheck>
<cardIdBeginBit>
<!--ro, req, int, start bit of the card No.-->1
</cardIdBeginBit>
<cardIdLength>
<!--ro, req, int, card No. length-->1
</cardIdLength>
<siteCodeBeginBit>
<!--ro, req, int, start bit of the site code-->1
</siteCodeBeginBit>
<siteCodeLength>
<!--ro, req, int, site code length-->1
</siteCodeLength>
<oemBeginBit>
<!--ro, req, int, start bit of OEM-->1
</oemBeginBit>
<oemLength>
<!--ro, req, int, OEM length-->1
</oemLength>
<manufacturerCodeBeginBit>
<!--ro, req, int, start bit of the manufacturer code-->1
</manufacturerCodeBeginBit>
<manufacturerCodeLength>
<!--ro, req, int, manufacturer code length-->1
</manufacturerCodeLength>
</CustomerCardIn>
<CustomerCardOut>
<!--ro, opt, object-->
...
<type>
<!--ro, req, enum, type, subType:string, desc:type-->cardId
</type>
<length>
<!--ro, req, int-->1
</length>
...
```
<name>
<!--req, string, Wiegand name-->test
</name>
<CustomerCardIn>
<!--opt, object-->
<totalLength>
<!--req, int, total Wiegand length, desc:when this node is set to 0, the custom Wiegand rule settings will be cleared-->1
</totalLength>
<checkMethod>
```
<!--req, enum, parity mode, subType:string, desc:parityCheck, xorCheck, noCheck;-->parityCheck
```
</checkMethod>
<ParityCheck>
<!--opt, object, configuration rule of odd-even parity, desc:this node is valid when <checkMethod> is "parityCheck”-->
<oddBeginBit>
<!--opt, int, start bit of odd parity-->1
</oddBeginBit>
<oddLength>
<!--opt, int, odd parity length-->1
</oddLength>
<evenBeginBit>
<!--opt, int, start bit of even parity-->1
</evenBeginBit>
<evenLength>
<!--opt, int, even parity length-->1
</evenLength>
</ParityCheck>
<XorCheck>
<!--opt, object, configuration rule of XOR parity, desc:this node is valid when <checkMethod> is "xorCheck”-->
<xorBeginBit>
<!--opt, int, start bit of XOR parity-->1
</xorBeginBit>
<xorPerLength>
<!--opt, int, length of each XOR parity group-->1
</xorPerLength>
<xorTotalLength>
<!--opt, int, total length of XOR parity data-->1
</xorTotalLength>
</XorCheck>
<cardIdBeginBit>
<!--req, int, start bit of the card No.-->1
</cardIdBeginBit>
<cardIdLength>
<!--req, int, card No. length-->1
</cardIdLength>
<siteCodeBeginBit>
<!--req, int, start bit of the site code-->1
</siteCodeBeginBit>
<siteCodeLength>
<!--req, int, site code length-->1
</siteCodeLength>
<oemBeginBit>
<!--req, int, start bit of OEM-->1
</oemBeginBit>
<oemLength>
<!--req, int, OEM length-->1
</oemLength>
<manufacturerCodeBeginBit>
<!--req, int, start bit of the manufacturer code-->1
</manufacturerCodeBeginBit>
<manufacturerCodeLength>
<!--req, int, manufacturer code length-->1
</manufacturerCodeLength>
</CustomerCardIn>
<CustomerCardOut>
<!--opt, object-->
...
<type>
<!--req, enum, module type, subType:string, desc:module type-->cardId
</type>
<length>
<!--req, int-->1
</length>
...
```
<requestURL>
<!--ro, req, string, request URL-->test
</requestURL>
<statusCode>
<!--ro, req, int, status code-->1
</statusCode>
<statusString>
<!--ro, req, string, status description-->test
</statusString>
<subStatusCode>
<!--ro, req, string, sub status code-->test
</subStatusCode>
</ResponseStatus>
Request URL
...
None
Response Message
11.4.7.25 Get the configuration capability of the custom Wiegand rule
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<WiegandRuleCfg xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
<name min="1" max="10">
```
<!--ro, req, string, Wiegand name, range:[1,32], attr:min{req, int},max{req, int}-->test
```
</name>
<CustomerCardIn>
<!--ro, opt, object-->
...
```
<!--ro, req, int, total Wiegand length., attr:min{req, int},max{req, int}-->1
```
</totalLength>
<checkMethod opt="parityCheck,xorCheck,noCheck,">
```
<!--ro, req, enum, parity mode, subType:string, attr:opt{req, string}, desc:parity mode-->parityCheck
```
</checkMethod>
<ParityCheck>
<!--ro, opt, object, configuration rule of odd-even parity, desc:configuration rule of odd-even parity-->
<oddBeginBit min="1" max="10">
```
<!--ro, opt, int, start bit of odd parity, attr:min{req, int},max{req, int}-->1
```
</oddBeginBit>
<oddLength min="1" max="10">
```
<!--ro, opt, int, odd parity length, attr:min{req, int},max{req, int}-->1
```
</oddLength>
<evenBeginBit min="1" max="10">
```
<!--ro, opt, int, start bit of even parity, attr:min{req, int},max{req, int}-->1
```
</evenBeginBit>
<evenLength min="1" max="10">
```
<!--ro, opt, int, even parity length, attr:min{req, int},max{req, int}-->1
```
</evenLength>
</ParityCheck>
<XorCheck>
<!--ro, opt, object, configuration rule of XOR parity, desc:configuration rule of XOR parity-->
<xorBeginBit min="1" max="10">
```
<!--ro, opt, int, start bit of XOR parity, attr:min{req, int},max{req, int}-->1
```
</xorBeginBit>
<xorPerLength min="1" max="10">
```
<!--ro, opt, int, length of each XOR parity group, attr:min{req, int},max{req, int}-->1
```
</xorPerLength>
<xorTotalLength min="1" max="10">
```
<!--ro, opt, int, total length of XOR parity data, attr:min{req, int},max{req, int}-->1
```
</xorTotalLength>
</XorCheck>
<cardIdBeginBit min="1" max="10">
```
<!--ro, req, int, start bit of the card No., attr:min{req, int},max{req, int}-->1
```
</cardIdBeginBit>
<cardIdLength min="1" max="10">
```
<!--ro, req, int, card No. length, attr:min{req, int},max{req, int}-->1
```
</cardIdLength>
<siteCodeBeginBit min="1" max="10">
```
<!--ro, req, int, start bit of the site code, attr:min{req, int},max{req, int}-->1
```
</siteCodeBeginBit>
<siteCodeLength min="1" max="10">
```
<!--ro, req, int, site code length, attr:min{req, int},max{req, int}-->1
```
</siteCodeLength>
<oemBeginBit min="1" max="10">
```
<!--ro, req, int, start bit of OEM, attr:min{req, int},max{req, int}-->1
```
</oemBeginBit>
<oemLength min="1" max="10">
```
<!--ro, req, int, OEM length, attr:min{req, int},max{req, int}-->1
```
</oemLength>
<manufacturerCodeBeginBit min="1" max="10">
```
<!--ro, req, int, start bit of the manufacturer code, attr:min{req, int},max{req, int}-->1
```
</manufacturerCodeBeginBit>
<manufacturerCodeLength min="1" max="10">
```
<!--ro, req, int, manufacturer code length, attr:min{req, int},max{req, int}-->1
```
</manufacturerCodeLength>
</CustomerCardIn>
<CustomerCardOut>
...
<No min="1" max="10">
```
<!--ro, req, int, No., attr:min{req, int},max{req, int}-->1
```
</No>
<type opt="cardId,siteCode,oem,manufacturerCode">
```
<!--ro, req, enum, type, subType:string, attr:opt{req, string}, desc:type-->cardId
```
</type>
<length min="1" max="10">
```
...
11.4.8 Multi-Door Interlocking
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
```
11.4.8.1 Get the capability of configuring the multi-door interlocking
11.4.8.2 Get the multi-door interlocking configuration parameters
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
```
Request URL
11.4.8.3 Set multi-door interlocking parameters
11.4.9 Multi-Factor Authentication
11.4.9.1 Get the capability of clearing group parameters
11.4.9.2 Clear group parameters
Hikvision co MMC
adil@hikvision.co.az
...
11.4.9.3 Get the group configuration parameters
11.4.9.4 Set the group parameters
Hikvision co MMC
adil@hikvision.co.az
Query Parameter
Parameter Name Parameter Type Description
...
None
Response Message
11.4.9.5 Get the group configuration capability
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.9.6 Get the configuration parameters of multi-factor authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
doorID string --
Request Message
11.4.9.7 Set parameters of multi-factor authentication mode
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.9.8 Get the configuration capability of multi-factor authentication modeHikvision co MMC
adil@hikvision.co.az
```
{
```
...
Query Parameter
11.4.10 Permission Schedules for Persons and Access Points
11.4.10.1 Set the holiday group parameters of the access permission control schedule
Hikvision co MMC
adil@hikvision.co.az
Parameter Name Parameter Type Description
holidayGroupID string --
...
```
11.4.10.2 Get the holiday group configuration parameters of the access permission control schedule
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
holidayPlanID string --
Request Message
11.4.10.3 Get the holiday group configuration capability of the access permission control
11.4.10.4 Set the holiday schedule parameters of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.10.5 Get holiday schedule configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.10.6 Get the holiday schedule configuration capability of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.10.7 Get the schedule template configuration parameters of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Request Message
None
11.4.10.8 Set the schedule template parameters of the access permission control
11.4.10.9 Get the schedule template configuration capability of the access permission control
Hikvision co MMC
adil@hikvision.co.az
Response Message
```
...
None
Response Message
11.4.10.10 Get weekly schedule configuration parameters
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
Response Message
11.4.10.11 Set the week schedule parameters of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.10.12 Get the weekly schedule configuration capability of the access permission control
Hikvision co MMC
adil@hikvision.co.az
```
{
...
None
Response Message
11.4.11 Person and Credential Synchronized Management
```
11.4.11.1 Get the capability of deleting person information (including linked cards, fingerprints, and faces)
```
and permissions
Hikvision co MMC
adil@hikvision.co.az
...
```
11.4.11.2 Start deleting all person information (including linked cards, fingerprints, and faces) and
```
permissions by employee No.
```
11.4.11.3 Get the status of deleting all person information (including linked cards, fingerprints, and faces)
```
...
None
Request Message
11.4.12 Person and Credential Synchronized Management
```
11.4.12.1 Get the capability of deleting person information (including linked cards, fingerprints, and faces)
```
and permissions
```
11.4.12.2 Start deleting all person information (including linked cards, fingerprints, and faces) and
```
permissions by employee No.
Hikvision co MMC
adil@hikvision.co.az
...
taskID string --
```
11.4.12.3 Get the status of deleting all person information (including linked cards, fingerprints, and faces)
```
and permissions by employee No
11.4.13 Person and Credential Task Management
11.4.13.1 Delete a specified task of applying person data asynchronously
Hikvision co MMC
adil@hikvision.co.az
Request Message
None
...
"enable": {
```
11.4.13.2 Get the capabilities of applying person data asynchronously
Hikvision co MMC
adil@hikvision.co.az
```
"enable": {
...
None
Response Message
11.4.13.3 Get the status of all tasks of applying person data asynchronously
Hikvision co MMC
adil@hikvision.co.az
```
{
...
Request Message
11.4.14 Door Status Control
11.4.14.1 Remotely control the door or elevator
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
...
None
Response Message
11.4.14.2 Get the capability set of remote door status control
11.5 Credentials Collection
11.5.1 Card Online Collection
11.5.1.1 Get the capability of collecting card information
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
Request URL
GET /ISAPI/AccessControl/CaptureCardInfo?format=json&readerID=<readerID>&timeout=<timeout>
Query Parameter
Parameter Name Parameter Type Description
readerID string --
timeout string --
...
11.5.1.2 Collect card information by the card reading module of the device
11.5.2 Fingerprint Online Collection
11.5.2.1 Collect fingerprint information
Hikvision co MMC
adil@hikvision.co.az
Response Message
<?xml version="1.0" encoding="UTF-8"?>
...
transmitted data of form unit is file, and it refers to the file name of form unit body.
Parameter Value: the body content of form unit.
11.5.2.2 Get the fingerprint collection capability
Hikvision co MMC
adil@hikvision.co.az
<?xml version="1.0" encoding="UTF-8"?>
<CaptureFingerPrint xmlns="http://www.isapi.org/ver20/XMLSchema" version="2.0">
...
Request Message
```
11.6 Security Control Device (General)
```
11.6.1 Event Message Push Management
11.6.1.1 Get the parameters of the report uploading method
11.6.1.2 Set the parameters of the report uploading method
Hikvision co MMC
adil@hikvision.co.az
```
{
...
```
11.6.1.3 Get the configuration capability of the report uploading method
Hikvision co MMC
adil@hikvision.co.az
Request URL
...
/*ro, opt, array, options, subType:string*/
11.7 Zone Alarm
11.7.1 Alarm System Management
11.7.1.1 Get the capability of security control panel
Hikvision co MMC
adil@hikvision.co.az
```
},
...
None
Response Message
11.7.1.2 Get the configuration capability of security control panel
Hikvision co MMC
adil@hikvision.co.az
```
{
...
"isSptControlAlarmChan": true,
```
/*ro, opt, bool, whether it supports arming or disarming the alarm input port (zone), desc:/ISAPI/SecurityCP/ControlAlarmChan?format=json*/
```
```
}
```
