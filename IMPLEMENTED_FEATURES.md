...
**Выезд в режиме «по белому списку»** имеет свой подрежим — `parking.listExitMode`
(служебное окно парковки, выбор появляется, когда выбран подрежим «По белому списку»):

- `Session` (по умолчанию, прежнее поведение) — выпускаем только машину с открытой сессией,
  то есть ту, которая числится заехавшей. Без сессии в журнал идёт `denied`
  «Exit without an open session», шлагбаум закрыт.
- `List` — факт въезда не проверяется: выпускаем любой номер из белого списка
  (непросроченная лицензия) или с действующим абонементом, даже если сессии нет.
  В журнал пишется `barrier_open` «exit (whitelist, no session)». Чёрный список
  по-прежнему закрывает шлагбаум. Если открытая сессия есть — она закрывается как обычно.

Подрежим действует только в бесплатном режиме «по белому списку»: в платном и «по местам»
выезд всегда идёт по сессии, иначе нечего тарифицировать и поедет счётчик занятости.
На выездной камере при этом нужно **явно задать направление «Выезд»** — автоопределение
направления опирается на наличие открытой сессии и без неё примет машину за въезжающую.
