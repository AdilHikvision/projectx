...
Пользователь **не создаётся из конфигурации**. При пустой таблице пользователей UI перенаправляет на **Initial Setup** (`/setup-password`): администратор задаёт **email и пароль** сам. После этого вход — через обычный `/login`.
