    ...
    ILogger<EventListenerService> logger) : BackgroundService, IEventListenerService
{
    ...
    public void Publish(DeviceEvent deviceEvent) => Emit(deviceEvent);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (active.Count > 0)
                {
                    ...
                    {
                        Emit(MapEvent(rawEvent), stoppingToken);
                    }
    ...
    private void Emit(DeviceEvent mapped, CancellationToken cancellationToken = default)
    {
        Enqueue(mapped);
        _ = EmitBroadcastAsync(mapped, cancellationToken);
    }
